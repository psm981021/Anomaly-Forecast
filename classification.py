...
# 모델 초기화
def initialize_model(learning_rate, loss_type):
...
if __name__ == "__main__":
    ...
    model, criterion, optimizer = initialize_model(learning_rate, loss_type)
    train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, 'cf_models/classification_model_seoul2')
    evaluate_model(model, val_loader)
    save_model(model, 'cf_models/classification_model_seoul1.pth')

    loaded_model = load_model('cf_models/classification_model_seoul1.pth')
    predict(loaded_model, test_loader)

    # # 예측 수행
    # image_path = 'radar_total/202105302300.png'
    # prediction = inference(loaded_model, image_path, transform)
