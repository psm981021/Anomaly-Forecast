...
class SignalDataset(Dataset):
    ...
    def make_signal_list(self):
        ...
        for i in range(-50, 50):
            signal_list.append('강수량(mm)'+str(i))
        return signal_list

    def make_rolling_signals(self):
        for i in range(-50, 50):
            self.signal_df['강수량(mm)'+str(i)] = self.signal_df['강수량(mm)'].shift(i)
    ...
    def __getitem__(self, idx):
        ...
        return {'강수량(mm)':x, 'anomaly':row['anomaly']}

def critic_x_iteration(sample):
    optim_cx.zero_grad()

    x = sample['강수량(mm)'].view(1, batch_size, signal_shape).to(device)
    valid_x = critic_x(x).to(device)
    valid_x = torch.squeeze(valid_x).detach().cpu()
    # 
    ...
    x_ = decoder(z)
    fake_x = critic_x(x_)
    fake_x = torch.squeeze(fake_x).detach().cpu()
...
def critic_z_iteration(sample):
    ...
    z = encoder(x)
    valid_z = critic_z(z)
    valid_z = torch.squeeze(valid_z).detach().cpu()
    ...
    fake_z = critic_z(z_)
    fake_z = torch.squeeze(fake_z).detach().cpu()
...
if __name__ == "__main__":
    ...
    # list=[400,401,402,408,421,569]
    
    # for i in list:
    #     name='data/'+str(i)+'_aws_data.csv'



    dataset = pd.read_csv('test_data_withanomaly.csv')
    #Splitting intro train and test
    #TODO could be done in a more pythonic way
    train_len = int(0.7 * dataset.shape[0])
    dataset[0:train_len].to_csv('train_realdataset.csv', index=False)
    dataset[train_len:].to_csv('test_realdataset.csv', index=False)

    train_dataset = SignalDataset(path='train_realdataset.csv')
    test_dataset = SignalDataset(path='test_realdataset.csv')
    batch_size = 64
    ...
    signal_shape = 100
    latent_space_dim = 20
    encoder_path = 'TadGAN/models/encoder.pt'
    decoder_path = 'TadGAN/models/decoder.pt'
    critic_x_path = 'TadGAN/models/critic_x.pt'
    critic_z_path = 'TadGAN/models/critic_z.pt'
    ...
    anomaly_detection.test(test_loader, encoder, decoder, critic_x,device)
