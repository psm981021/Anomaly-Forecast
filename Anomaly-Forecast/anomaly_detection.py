...
def test(test_loader, encoder, decoder, critic_x,device):
    ...
    for batch, sample in enumerate(tqdm(test_loader)):
        reconstructed_signal = decoder(encoder(sample['강수량(mm)'].to(device)))
        reconstructed_signal = torch.squeeze(reconstructed_signal)

        for i in range(0, 64):
            x_ = reconstructed_signal[i].cpu().detach().numpy()
            x = sample['강수량(mm)'][i].numpy()
            y_true.append(int(sample['anomaly'][i].detach()))
            
            reconstruction_error.append(dtw_reconstruction_error(x, x_))
        
        critic_score.extend(torch.squeeze(critic_x(sample['강수량(mm)'].to(device))).cpu().detach().numpy())

       # import IPython; IPython.embed(colors='Linux'); exit(1)
        # plt.plot(sample['강수량(mm)'][0].detach().numpy())
        # plt.plot(reconstructed_signal[0].detach().numpy())
    # import IPython; IPython.embed(colors='Linux'); exit(1)
